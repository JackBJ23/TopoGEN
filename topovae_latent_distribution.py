# -*- coding: utf-8 -*-
"""topovae_fashionmnist_github2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/11IE6_a5H6feMSZxSn0qHTxeeyuXOqbED
"""
...
for epoch in range(n_epochs):
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        if batch_idx % n_showplots != 0 or batch_idx==0:
          ...
          recon_batch, mean, log_var, _ = model0(data)
...
for epoch in range(1):
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        if batch_idx % n_showplots != 0 or batch_idx==0:
          ...
          optimizer0.step()
          #model2
          recon_batch, mean, log_var, _  = model2(data)
    ...
    plt.plot(np.arange(len(losses)), losses, label='Conv-VAE')
    plt.plot(np.arange(len(lossestopo)), lossestopo, label='Conv-TopoVAE')
    plt.plot(np.arange(len(lbottleneck0)), lbottleneck0, label='Lbottleneck0')
    plt.plot(np.arange(len(lbottleneck1)), lbottleneck1, label='Lbottleneck0')
    plt.xlabel("Iteration")
    plt.ylabel("BCE loss")
    plt.legend(loc='upper right')
    plt.show()
...
"""Added: using n_latent=2, take 1000 points from both models and compare the distribution of the latent space."""
...
"""Same, but with a scheduler for decay of topoloss:"""
...
for epoch in range(1):
    for batch_idx, (data, _) in enumerate(train_loader):
        w0 = w0 * np.exp(-float(batch_idx) * 0.05)
        if batch_idx%n_showplots==0 and batch_idx>0:
          plt.plot(np.arange(len(losses)), losses, label='Conv-VAE')
          plt.plot(np.arange(len(lossestopo)), lossestopo, label='Conv-TopoVAE')
          plt.xlabel("Iteration")
        ...
        if batch_idx % n_showplots != 0 or batch_idx==0:
          ...
          dgm = get_dgm(recon_batch.view(data.size(0), -1), 1)
          ## replace the next line by the topo-loss of choice (or combination of topo-losses):
          loss = loss_fctn2(recon_batch, data, mean, log_var, dgm, dgm2, w0, w0)
          #loss = loss_fctn5(recon_batch, data, mean, log_var, dgm, dgm2, 1., 1.)
          loss.backward()
          optimizer2.step()
...
le0 = []
for x in lbottleneck0: le0.append(x*10000)
...
for batch_idx, (data, _) in enumerate(train_loader):
      ...
      dgm = get_dgm(recon_batch.view(data.size(0), -1), 1)
      ## replace the next line by the topo-loss of choice (or combination of topo-losses):
      loss = loss_fctn3(recon_batch, data, mean, log_var, dgm, dgm2, 3.5, 3.5, 0.1)
      #loss = loss_fctn5(recon_batch, data, mean, log_var, dgm, dgm2, 1., 1.)
      loss.backward()
      optimizer2.step()
      if batch_idx==10: break
...
for batch_idx, (data, _) in enumerate(train_loader):
      ...
      dgm = get_dgm(recon_batch.view(data.size(0), -1), 1)
      ## replace the next line by the topo-loss of choice (or combination of topo-losses):
      loss = loss_fctn4(recon_batch, data, mean, log_var, dgm, dgm2, 5., 10., 0.1)
      #loss = loss_fctn5(recon_batch, data, mean, log_var, dgm, dgm2, 1., 1.)
      loss.backward()
      optimizer2.step()
      if batch_idx==10: break
...
for batch_idx, (data, _) in enumerate(train_loader):
      ...
      dgm = get_dgm(recon_batch.view(data.size(0), -1), 1)
      ## replace the next line by the topo-loss of choice (or combination of topo-losses):
      loss = loss_fctn5(recon_batch, data, mean, log_var, dgm, dgm2, 5., 10.)
      #loss = loss_fctn5(recon_batch, data, mean, log_var, dgm, dgm2, 1., 1.)
      loss.backward()
      optimizer2.step()
      if batch_idx==10: break
...
"""Extra code: (not needed) can be used for tuning the parameters of loss_density (sigma, scale, range, number of points used) for a specific dataset. First take a batch of the dataset, and then plot curves for different parameters"""

# first get a batch of imgs (see it as pt cloud):
for batch_idx, (data, _) in enumerate(train_loader):
  data = data.view(data.size(0), -1)
  points_np = data.view(-1, img_size).numpy()
  if batch_idx==5:
...
def density(point_cloud, dgm, x, sigma, scale):
  density_x = 0 #density at x
  for i in range(len(dgm['dgms'][0])-1):
    p1, p2 = point_cloud[dgm['gens'][0][i][1]], point_cloud[dgm['gens'][0][i][2]] #pt (0,d) with d=dist(p1,p2) (euclidean dist)
    d = dist(p1, p2) #pt of pt cloud is (0,d)
...
def get_density_fctn(point_cloud, dgm, sigma, scale):
   n = 30
   density_fctn = np.zeros((n, 2))
   xs = np.linspace(0.,15.,n)
   for i in range(n):
    density_fctn[i][0] = density(point_cloud, dgm, xs[i], sigma, scale)
    density_fctn[i][1] = xs[i]
   return density_fctn

def get_curve(point_cloud, dgm, sigma, scale):
  arr = get_density_fctn(point_cloud, dgm, sigma, scale)
  ...
  scatter_trace = go.Scatter(x=y_coords, y=x_coords, mode='lines', name=f'4SGDE; σ={sigma}')

  return scatter_trace

point_cloud = torch.tensor(ptc, dtype=torch.float32, requires_grad=False)
dgm = ripser_parallel(ptc, maxdim=1, return_generators=True)
dgm_gtda = _postprocess_diagrams([dgm["dgms"]], "ripser", (0,), np.inf, True)[0]
fig = go.Figure(plot_diagram(dgm_gtda, homology_dimensions=(0,)))

curve_density1 = get_curve(point_cloud, dgm, 0.05, 0.0005)
fig.add_trace(curve_density1)
curve_density2 = get_curve(point_cloud, dgm, 0.1, 0.0005)
fig.add_trace(curve_density2)
curve_density3 = get_curve(point_cloud, dgm, 0.3, 0.0005)
