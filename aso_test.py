# -*- coding: utf-8 -*-
"""aso_tests_github.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1A05gxEvaQKHohZnofZ9bfmuU0EqYDCOU
"""
...
"""Train and compare model0 (normal VAE) and model2 (same structure but with topo-loss) and use the ASO test to compare their performance:"""
...
for seed in seeds: #20 no, 100 done, 15 done, 12 and 8 done (worse). seeds: int(w)
  ...
  while kt==1 and epoch<10:
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        if lossestopo[-1] < best_val_loss:
            best_val_loss = lossestopo[-1]
            epochs_without_improvement = 0
        else:
...
plt.plot(xs, bce_end_m0, label='I=last, VAE0')
plt.plot(xs, bce_end_m2, label='I=last, TopoVAE1')
plt.xlabel("Model")
plt.ylabel("BCE loss")
plt.legend(loc='upper right')
plt.show()

"""Comparison using 'Scenario 1 - Comparing multiple runs of two models' using ASO test, from https://deep-significance.readthedocs.io/en/latest/#scenario-1---comparing-multiple-runs-of-two-models"""
...
vae0_res = []
vae2_res = []
for x in bce_25_m2: vae2_res.append(-x)
for x in bce_25_m0: vae0_res.append(-x)
my_model_scores = vae2_res
baseline_scores = vae0_res
min_eps = aso(my_model_scores, baseline_scores, seed=1234)

print("at 40 iters", min_eps)
...
vae0_res = []
vae2_res = []
for x in bce_50_m2: vae2_res.append(-x)
for x in bce_50_m0: vae0_res.append(-x)
my_model_scores = vae2_res
baseline_scores = vae0_res
min_eps = aso(my_model_scores, baseline_scores, seed=1234)

print("at 50 iters", min_eps)
...
vae0_res = []
vae2_res = []
for x in bce_end_m2: vae2_res.append(-x)
for x in bce_end_m0: vae0_res.append(-x)
my_model_scores = vae2_res
baseline_scores = vae0_res
min_eps = aso(my_model_scores, baseline_scores, seed=1234)
...
'''
## code to show that both models have the exact same parameters when created:
...
def have_identical_initial_weights(model1, model2):
    state_dict1 = model1.state_dict()
    state_dict2 = model2.state_dict()

    for key in state_dict1:
        if key in state_dict2:
            if not torch.allclose(state_dict1[key], state_dict2[key], atol=1e-5):
                return False
        else:
            return False
...
if have_identical_initial_weights(model0, model2):
    print("The models have the exact same initial weights and biases.")
else:
    print("The models have different initial weights and biases.")
